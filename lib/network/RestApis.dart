...
Future<LoginResponse> login(Map request, {bool isSocialLogin = false}) async {
  Response response = await buildHttpResponse(isSocialLogin ? 'https://veekshanam.com/wp-json/wp/v2/users/me' : 'jwt-auth/v1/token', request: request, method: HttpMethod.POST);
...
Future<DashboardResponse> getDashboardApi(Map request, int page) async {
  if (!(await isNetworkAvailable()) && getStringAsync(DASHBOARD_DATA).isNotEmpty) {
   ...
   return await handleResponse(await buildHttpResponse('https://veekshanam.com/wp-json/wp/v2/posts?paged=$page', request: request, method: HttpMethod.POST)).then((value) async {
...
Future<NewsData> getBlogDetail(Map request, bool isLogin) async {
  return NewsData.fromJson(await (handleResponse(await buildHttpResponse('https://veekshanam.com/wp-json/wp/v2/posts/{post_id}', request: request, method: HttpMethod.POST))));
